// Activity & observability: timeline, offline recovery, diagnostics.
import { useEffect, useState } from 'react';
import { T } from '../design/tokens';
import { DEVICE_ICON, Icon, type IconName } from '../components/Icon';
import { Btn, Card, EmptyState, Mono, Sheet, StatusDot, TopBar, haptic, pressable } from '../components/ui';
import { Troll } from '../components/Troll';
import { useStore, useTweaks, VARKEY } from '../store/store';
import type { ActivityEvent, Device } from '../types';

const STATUS: Record<string, { c: string; ring: string; label: string; icon: IconName }> = {
...
const KIND: Record<string, string> = { scene: 'Scene', command: 'Command', scout: 'Troll Scout' };
...
function ActivityRow({ ev, last }: { ev: ActivityEvent; last: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 14, padding: '13px 2px', borderBottom: last ? 'none' : `1px solid ${T.border}` }}>
      <div style={{ position: 'relative', flexShrink: 0 }}>
        <div style={{ width: 40, height: 40, borderRadius: 13, background: T.violetSoft, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Icon name={ev.icon || (ev.kind === 'scout' ? 'bolt' : 'remote')} size={20} color={T.lime} />
        </div>
        <div
          style={{
            position: 'absolute',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 3 }}>
          <span style={{ fontFamily: T.mono, fontSize: 10, color: st.c, fontWeight: 700, letterSpacing: 0.5, textTransform: 'uppercase' }}>
            {KIND[ev.kind] || ev.kind} · {st.label}
          </span>
        </div>
...
export function Activity({ onDiagnostics, onRecover }: { onDiagnostics: () => void; onRecover: (d: Device) => void }) {
    ...
    <div style={{ padding: '4px 20px 20px' }}>
      <TopBar
        ...
        right={
          ...
          >
            <Icon name="ir" size={15} color={T.lime} /> Diagnostics
          </button>
        }
      />
      ...
      <Card pad={14} style={{ marginBottom: 14, display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ display: 'flex', gap: 18 }}>
          <div>
            <div style={{ fontWeight: 900, fontSize: 24, color: T.success, lineHeight: 1 }}>{online}</div>
            <Mono style={{ fontSize: 10.5, color: T.muted }}>ONLINE</Mono>
          </div>
          <div style={{ width: 1, background: T.border }} />
          <div>
            <div style={{ fontWeight: 900, fontSize: 24, color: offline.length ? T.error : T.muted, lineHeight: 1 }}>{offline.length}</div>
            <Mono style={{ fontSize: 10.5, color: T.muted }}>OFFLINE</Mono>
          </div>
        </div>
        <div style={{ flex: 1 }} />
        ...
        <Card key={d.id} pad={14} style={{ marginBottom: 14, borderColor: 'rgba(248,113,113,0.4)', background: 'rgba(248,113,113,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(248,113,113,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <Icon name={DEVICE_ICON[d.type]} size={20} color={T.error} />
            </div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontWeight: 800, fontSize: 15 }}>{d.name} is offline</div>
              <Mono style={{ fontSize: 11, color: T.muted }}>last seen {d.lastSeen || '—'}</Mono>
            </div>
            <Btn kind="danger" onClick={() => onRecover(d)} style={{ padding: '10px 16px', fontSize: 14 }}>
              Fix
            </Btn>
          </div>
        </Card>
      ...
      {groups.length === 0 ? (
        <div style={{ border: `1px dashed ${T.border}`, borderRadius: 20 }}>
          <EmptyState
            exp="happy"
            variant={trollVariant}
            compact
            title={filter === 'Failures' ? 'No failures' : 'Nothing yet'}
            sub={filter === 'Failures' ? 'Every command has gone through cleanly.' : 'Fire a scene and it shows up here.'}
          />
        </div>
      ) : (
        groups.map((g) => (
          <div key={g.label} style={{ marginBottom: 18 }}>
            ...
            <Card pad={16}>
              {g.items.map((ev, i) => (
                <ActivityRow key={ev.id} ev={ev} last={i === g.items.length - 1} />
              ))}
            </Card>
          </div>
        ))
...
// ---------- OFFLINE RECOVERY ----------
export function Recovery({ device, onClose }: { device: Device | null; onClose: () => void }) {
  const { reconnectDevice } = useStore();
  const trollVariant = VARKEY[useTweaks((s) => s.trollRendering)];
  const [phase, setPhase] = useState<'idle' | 'trying' | 'success'>('idle');
  ...
  return (
    <Sheet open={!!device} onClose={onClose}>
      {phase === 'idle' && (
        <div style={{ textAlign: 'center', padding: '4px 8px 8px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 10 }}>
            <Troll exp="sleepy" variant={trollVariant} presence="subtle" size={92} />
          </div>
          ...
          <p style={{ color: T.muted, fontWeight: 600, fontSize: 13.5, margin: '8px auto 18px', maxWidth: 280, lineHeight: 1.5 }}>
            Last seen {device.lastSeen || 'a while ago'}. Common fixes for the {device.model}:
          </p>
          <div style={{ textAlign: 'left', marginBottom: 18 }}>
            {['Confirm it has power and the LED is lit', 'Check it’s on the same Wi-Fi as your phone', `Verify the IP ${device.ip} hasn’t changed`].map((tx, i) => (
              <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '8px 0' }}>
                ...
                <span style={{ fontWeight: 700, fontSize: 14, color: 'rgba(240,240,240,0.9)', lineHeight: 1.4 }}>{tx}</span>
              </div>
            ))}
          </div>
          <Btn
          ...
          >
            <Icon name="refresh" size={17} /> Try to reconnect
          </Btn>
        </div>
      )}
      ...
      {phase === 'trying' && (
        <div style={{ textAlign: 'center', padding: '14px 8px 18px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 18, position: 'relative' }}>
            ...
            <Troll exp="wink" variant={trollVariant} presence="subtle" size={96} />
          </div>
      ...
      {phase === 'success' && (
        <div style={{ textAlign: 'center', padding: '10px 8px 14px' }}>
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 10 }}>
            <Troll exp="wow" variant={trollVariant} presence="subtle" size={96} glow />
          </div>
          <div style={{ fontWeight: 900, fontSize: 20 }}>Back online</div>
          <p style={{ color: T.muted, fontWeight: 600, fontSize: 13.5, margin: '8px auto 18px', maxWidth: 260 }}>
            {device.name} responded on {device.ip}. You’re good to go.
          </p>
          <Btn
            kind="lime"
            full
...
// ---------- DIAGNOSTICS ----------
export function Diagnostics({ open, onClose, onRecover }: { open: boolean; onClose: () => void; onRecover: (d: Device) => void }) {
  const devices = useStore((s) => s.devices);
  if (!open) return null;
  const lat = (d: Device) =>
    d.status !== 'online'
      ? { label: '—', c: T.muted }
      : (d.latency ?? 0) < 30
        ? { label: d.latency + 'ms', c: T.success }
        : (d.latency ?? 0) < 60
          ? { label: d.latency + 'ms', c: T.warning }
          : { label: d.latency + 'ms', c: T.error };
  return (
    <Sheet open={open} onClose={onClose} title="Diagnostics">
          ...
          <Card key={d.id} pad={14} style={{ marginBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <Mono style={{ fontSize: 11, color: T.muted }}>{d.protocol}</Mono>
              </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              {(
                ...
                <div key={k} style={{ flex: 1, background: T.card2, borderRadius: 10, padding: '8px 10px' }}>
                  ...
                  <Mono style={{ fontSize: 13, color: c, fontWeight: 700 }}>{v}</Mono>
                </div>
              ))}
            </div>
            {d.status === 'offline' && (
              <Btn
                kind="danger"
                full
                style={{ marginTop: 10, padding: '9px' }}
                onClick={() => {
                  onClose();
                  onRecover(d);
                }}
              >
                <Icon name="refresh" size={15} /> Troubleshoot
              </Btn>
            )}
          </Card>
        );
